...
function setUjOldal() {
    ...
    //Csak rendszergazdáknak és moderátoroknak!
    $ErrorStr = '';
    if (($_SESSION['AktFelhasznalo'.'FSzint']>3) && (isset($_POST['submitUjOldalForm']))){  
...
function setOldalTorol() {
    ...
    if (($_SESSION['AktFelhasznalo'.'FSzint']>0) && (isset($_POST['submitOldalTorolVegleges'])))  {
        ...
        if ($ErrorStr == '') {
            ...
            //Üzenet a törlésről
            $ErrorStr = "A $ONev oldalt töröltük!";
            //Az aktuális oldal megszűnt, így visszalépünk szülőoldalához 
            getOldalData($SzOUrl);
        }
    }
    return $ErrorStr;
    ...
    if ($_SESSION['AktFelhasznalo'.'FSzint']>3){ // FSzint-et növelni, ha működik a felhasználókezelés!!! 
        ...
        if ((0<$OTipus) && ($OTipus<10)) {
            ...
            if ((!isset($_POST['submitOldalTorolForm'])) && (!isset($_POST['submitOldalTorolVegleges'])))  { 
                ...
                $HTMLkod .= "<p class='FontosStr'>Valóban törli a $ONev oldalt?! A művelet végleges!</p>";
            ...
            if (isset($_POST['submitOldalTorolForm']) &&  (strpos($_SESSION['ErrorStr'],'Err0')!==false))  {  
                ...
                if ($_SESSION['ErrorStr']=='Err001') {$HTMLkod .= "A $ONev oldal törlése előtt az oldal aloldalait törölni kell! ";}
            ...
            if (isset($_POST['submitOldalTorolForm']) &&  (strpos($_SESSION['ErrorStr'],'Err0')===false))  { 
                ...
                $HTMLkod .= "<div id='divOldalTorolForm' >\n";
                $HTMLkod .= "<form action='?f0=$OUrl' method='post' id='formOldalTorolForm'>\n"; 
                $HTMLkod .= "<p class='FontosStr'>Biztosan törli a(z) $ONev nevű oldalt? </p>";
                $HTMLkod .=  "<br><input type='submit' name='submitOldalTorolVegleges' value='Törlés'><br>\n";        
                $HTMLkod .= "</form>\n";              
                $HTMLkod .= "</div>\n";
            } 
